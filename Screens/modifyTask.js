...
export default function ModifyTask({ route, navigation }) {
  ...
  const [name, setName] = useState(item ? item.taskname : "");
  const [workflow, setWorkflow] = useState(item ? item.workplace_id : "");
  ...
  let formattedItemTime = item.endTime && item.endTime.split(":");

  //Time Picker states and functions
  const [time, setTime] = React.useState({
    hours: formattedItemTime ? +formattedItemTime[0] : 12,
    minutes: formattedItemTime ? +formattedItemTime[1] : 0,
  });
  ...
  const handleNewTask = async () => {
    ...
    modifyTask(
      token,
      {
        ...
        followers: followers.length > 0 ? followers : item.followers,
        createdBy: user._id,
        endTime,
  ...
  useEffect(() => {
    if (item) {
      dispatch(setTaskFollowers(item.followers));
    }
  }, [item]);
          ...
          <View style={{ flex: 1, justifyContent: "center", paddingLeft: 12 }}>
            <View style={{ flexDirection: "row" }}>
              ...
              <View style={{ paddingLeft: 20, flexDirection: "column" }}>
                <Text style={{ fontSize: 26 }}>Modify Task </Text>
            ...
            <View style={{ alignItems: "center" }}>
              <View style={{ flexDirection: "row", paddingTop: 80 }}>
                <TouchableOpacity style={styles.btn} onPress={handleNewTask}>
                  <Text style={styles.txt1}>Update Task </Text>
                </TouchableOpacity>
              </View>
            </View>
          </View>
