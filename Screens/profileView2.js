...
import getUserPendingTasks from "../utils/getPendingTasks";

export default function ProfileViewTwo({ route, navigation }) {
  ...
  React.useEffect(() => {
    ...
    async function getMyPendingTasks() {
      try {
        const token = await getToken();
        const tasks = await getUserPendingTasks(id, token);
